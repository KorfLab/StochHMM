...
namespace StochHMM {
	...
	void trellis::naive_viterbi(){
		...
		naive_viterbi_score = new (std::nothrow) double_2D(seq_size, std::vector<double>(state_size,-INFINITY));
		...
		for(size_t st = 0; st < state_size; ++st){
			...
			(*naive_viterbi_score)[0][st]=viterbi_temp;
		...
		for (size_t position = 1 ; position < seq_size ; ++position){
			for (size_t st_current = 0; st_current < state_size; ++st_current){
				...
				for (size_t st_previous = 0; st_previous < state_size; ++st_previous){
					previous = (*naive_viterbi_score)[position-1][st_previous];
					...
					if (trans !=-INFINITY){
						...
						if (viterbi_temp > (*naive_viterbi_score)[position][st_current]){
							(*naive_viterbi_score)[position][st_current] = viterbi_temp;
		...
		for (size_t st_previous = 0; st_previous < state_size; ++st_previous){
			if ((*hmm)[st_previous]->getEndTrans() != -INFINITY){
				if ((*naive_viterbi_score)[seq_size-1][st_previous] != -INFINITY){
					viterbi_temp = (*naive_viterbi_score)[seq_size-1][st_previous] + (*hmm)[st_previous]->getEndTrans();
