...
//  M82_PEN_HET
//
//  Created by Paul Lott on 8/10/13. Modified by Daniel Fulop in Oct. and Nov. 2013.
//  Copyright (c) 2013 Korf Lab, Genome Center, UC Davis, Davis, CA. All rights reserved.
//  Copyright (c) 2013 Maloof Lab, Plant Biology, UC Davis, Davis, CA. All rights reserved.
//
...
double m82_m82(const double pos, const std::vector<double>* distance){
	size_t position = static_cast<size_t>(pos);
	double d = (*distance)[position];
	double trans_value = (1+exp(-d/50))/2;
	//std::cout << "m82_m82 transition call:" << position << "\t" << (*distance)[position] << std::endl;
	return log(trans_value);
}

double m82_pen(const double pos, const std::vector<double>* distance){
	size_t position = static_cast<size_t>(pos);
	double d = (*distance)[position];
	double trans_value = (1-exp(-d/50))/4;// * 0.99;
	//std::cout << "m82_pen transition call:" << position << "\t" << (*values)[position] << std::endl;
	return log(trans_value);
}

double m82_het(const double pos, const std::vector<double>* distance){
	size_t position = static_cast<size_t>(pos);
	double d = (*distance)[position];
	double trans_value = (1-exp(-d/50))/4;// * 0.01;
	//std::cout << "m82_het transition call:" << position << "\t" << (*values)[position] << std::endl;
	return log(trans_value);
}

double pen_m82(const double pos, const std::vector<double>* distance){
	size_t position = static_cast<size_t>(pos);
	double d = (*distance)[position];
	double trans_value = (1-exp(-d/50))/4;// * 0.99;
...
double pen_pen(const double pos, const std::vector<double>* distance){
	size_t position = static_cast<size_t>(pos);
	double d = (*distance)[position];
	double trans_value = (1+exp(-d/50))/2;
...
double pen_het(const double pos, const std::vector<double>* distance){
	size_t position = static_cast<size_t>(pos);
	double d = (*distance)[position];
	double trans_value = (1-exp(-d/50))/4;// * 0.01;
...
double het_m82(const double pos, const std::vector<double>* distance){
	size_t position = static_cast<size_t>(pos);
	double d = (*distance)[position];
	double trans_value = (1-exp(-d/50))/4;
	//std::cout << "het_m82 transition call:" << position << "\t" << (*values)[position] << std::endl;
	return log(trans_value);
}

double het_pen(const double pos, const std::vector<double>* distance){
	size_t position = static_cast<size_t>(pos);
	double d = (*distance)[position];
	double trans_value = (1-exp(-d/50))/4;
...
double het_het(const double pos, const std::vector<double>* distance){
	size_t position = static_cast<size_t>(pos);
	double d = (*distance)[position];
	double trans_value = (1+exp(-d/50))/2;
