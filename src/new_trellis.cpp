...
//            transition_prob+=exFuncTraceback(trans->getExtFunction());
	...
	void trellis::naive_forward(){
		std::vector<std::vector<double> >* forw_table = new std::vector<std::vector<double> >(seq_size, std::vector<double>(state_size,-INFINITY));
		
		double emission(-INFINITY);
		double forward_temp(-INFINITY);
		double trans(-INFINITY);
		double previous(-INFINITY);
		double ending_prob(-INFINITY);
		...
		for (size_t position = 1 ; position < seq_size ; ++position){
			for (size_t st_current = 0; st_current < state_size; ++st_current){
				...
				for (size_t st_previous = 0; st_previous < state_size; ++st_previous){
					...
					previous = (*forw_table)[position-1][st_previous];
					
					if (trans !=-INFINITY && previous != -INFINITY){
						forward_temp = emission + trans + previous;
						
						if ((*forw_table)[position][st_current]==-INFINITY){
							(*forw_table)[position][st_current] = forward_temp;
						}
						else{
							(*forw_table)[position][st_current] = addLog((*forw_table)[position][st_current], forward_temp);
						}
					}
				}
			}
		...
		for (size_t st_previous = 0; st_previous < state_size; ++st_previous){
			if ((*hmm)[st_previous]->getEndTrans() != -INFINITY){
				if ((*forw_table)[seq_size-1][st_previous] != -INFINITY){
					ending_prob = addLog((*forw_table)[seq_size-1][st_previous], ending_prob);
				}
			}
		...
		std::cout.precision(dbl::digits10);
		std::cout << "Ending Forward Probability:\t" << ending_prob << std::endl;
		for(size_t i=0; i< seq_size ;i++){
			std::cout << i+1 << "\t";
			
			for (size_t j=0; j< state_size; j++){
				std::cout << (*forw_table)[i][j] << "\t";
			}
			std::cout << std::endl;
		}
	}
	...
	void trellis::naive_backward(){
		std::vector<std::vector<double> >* back_table = new std::vector<std::vector<double> >(seq_size, std::vector<double>(state_size,-INFINITY));
		...
		for(size_t st_current = 0 ; st_current < state_size; st_current++){
			...
			if (trans != -INFINITY){
				(*back_table)[seq_size-1][st_current] = trans;
			}
		}
		...
		for (size_t position = seq_size-2; position != SIZE_T_MAX ; position--){
			...
			for(size_t st_current = 0; st_current < state_size ; st_current++){
				...
				for (size_t st_previous = 0; st_previous < state_size; st_previous++){
					...
					previous = (*back_table)[position+1][st_previous];
					...
					if ((*back_table)[position][st_current] == -INFINITY){
						(*back_table)[position][st_current] = backward_temp;
					}
					else{
						(*back_table)[position][st_current] = addLog( backward_temp, (*back_table)[position][st_current]);
					}
		...
		state* init = hmm->getInitial();
		for (size_t st_current=0; st_current < state_size; st_current++ ){
			if ((*back_table)[0][st_current] == -INFINITY){
				continue;
			}
			
			previous = (*back_table)[0][st_current];
			...
			if (ending_prob == -INFINITY){
				ending_prob = backward_temp;
			}
			else{
				ending_prob = addLog(ending_prob, backward_temp);
			}
		}
		...
		std::cout.precision(dbl::digits10);
		std::cout << "Ending Backward Probability:\t" << ending_prob << std::endl;
		
		for(size_t i=0; i< seq_size ;i++){
			std::cout << i+1 << "\t";
			
			for (size_t j=0; j< state_size; j++){
				std::cout << (*back_table)[i][j] << "\t";
			}
			std::cout << std::endl;
		}
