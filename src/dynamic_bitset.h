...
namespace StochHMM {
	...
	public:
		...
		class bit_ref{
		public:
			bit_ref(uint32_t& v, size_t position):val(v),pos(position){}
			bit_ref& operator=(bool value);
			bit_ref& operator=(const bit_ref& rhs);
			
			bit_ref& operator|=(bool value); //OR
			bit_ref& operator&=(bool value); //AND
			bit_ref& operator^=(bool value); //XOR
			bit_ref& flip();
			bit_ref& reset();
			
			bool operator~() const;
			operator bool() const { return (this->val >> pos) & 1;}
		private:
			uint32_t& val;
			size_t pos;
		};
